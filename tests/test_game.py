...
async def test_two_player_boards_end_above_the_footer() -> None:
...
async def test_both_players_receive_independent_pieces_and_controls() -> None:
    app = TetrisApp(two_players=True)
...
async def test_single_player_mode_mounts_one_board() -> None:
    app = TetrisApp()
    async with app.run_test() as pilot:
        await pilot.pause()
...
async def test_hard_drop_allows_a_grounded_piece_to_be_adjusted_before_locking() -> None:
    app = TetrisApp()
    async with app.run_test() as pilot:
        await pilot.pause()
...
async def test_local_player_names_are_visible_and_changeable() -> None:
    app = TetrisApp(two_players=True)
    async with app.run_test() as pilot:
        await pilot.pause()
