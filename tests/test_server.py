...
async def test_headless_server_supports_agents_names_chat_and_queue() -> None:
...
async def test_next_queued_client_is_promoted_after_disconnect() -> None:
    server = TetrisServer("127.0.0.1", 0)
    async with server.running():
        url = f"ws://127.0.0.1:{server.port}"
...
async def test_watch_only_client_never_occupies_or_queues_for_a_player_slot() -> None:
    server = TetrisServer("127.0.0.1", 0)
    async with server.running():
        url = f"ws://127.0.0.1:{server.port}"
...
async def test_next_queued_client_is_promoted_after_game_over() -> None:
    server = TetrisServer("127.0.0.1", 0)
    async with server.running():
        url = f"ws://127.0.0.1:{server.port}"
        async with connect(url) as first, connect(url) as second, connect(url) as queued:
...
async def test_textual_client_receives_role_and_visible_name() -> None:
    server = TetrisServer("127.0.0.1", 0)
    async with server.running():
        url = f"ws://127.0.0.1:{server.port}"
...
async def test_embedded_server_waits_for_a_remote_player() -> None:
    server = TetrisServer("127.0.0.1", 0)
...
async def test_embedded_watch_only_server_leaves_both_player_slots_available() -> None:
    server = TetrisServer("127.0.0.1", 0)
    app = TetrisApp(network_mode="client", embedded_server=server, watch_only=True)
    async with app.run_test(size=(181, 59)) as pilot:
        deadline = asyncio.get_running_loop().time() + 2
...
async def test_loser_can_join_again_when_no_challenger_is_waiting() -> None:
    server = TetrisServer("127.0.0.1", 0)
    async with server.running():
        url = f"ws://127.0.0.1:{server.port}"
